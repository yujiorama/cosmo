...
export default (opts: BaseCommandOptions) => {
  ...
  command.description('Rejects an existing proposal for a federated graph.');
  command.argument('<name>', 'The name of the proposal to reject.');
  ...
  command.action(async (name, options) => {
    const spinner = ora(`Rejecting proposal: ${name}...`).start();

    const resp = await opts.client.platform.rejectProposal(
      {
        proposalName: name,
